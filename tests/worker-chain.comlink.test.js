...
describe("Comlink.wrapChain()", function() {
  beforeEach(function() {
  ...
  describe("Chain-specific", function() {
    it("supports both getters and method calls", async function() {
      const chainThing = Comlink.wrapChain(this.port1);
      Comlink.expose(chainSample, this.port2);
      const result = await chainThing()
        .method1()
        .prop1
        .prop2
        .method2()
    ...
    it("supports constructors and setters", async function() {
      let counter = 0;
      class ChainClass {
        constructor() {
          counter++;
      ...
      ChainClass.sum = () => counter;

      const ChainThing = Comlink.wrapChain(this.port1);
      Comlink.expose(ChainClass, this.port2);
      await (new ChainThing().setter = 2);

      expect(await ChainThing.sum()).to.equal(3);
    });
  });

  describe("basic wrap() functionality", function() {
    ...
    guardedIt(hasBroadcastChannel)(
      ...
      async function() {
        ...
        const thing = Comlink.wrapChain(b1);
    ...
    it("can handle destructuring", async function() {
      ...
      const { a, b, c } = Comlink.wrapChain(this.port1);
    ...
    it("lets users define transfer handlers", function(done) {
      ...
      const thing = Comlink.wrapChain(this.port2);
    ...
    it("can tunnel a new endpoint with createEndpoint", async function() {
      ...
      const proxy = Comlink.wrapChain(this.port1);
      const otherEp = await proxy[Comlink.createEndpoint]();
      const otherProxy = Comlink.wrapChain(otherEp);
